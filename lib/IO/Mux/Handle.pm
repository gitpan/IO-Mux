...
multiplexed through a L<IO::Mux> object.
...
Errors are reported using the standard return values and mechanisms. See 
below (L<IO::Mux::Handle/ERROR REPORTING>) for more details.
...
When error occur on the underlying (real) handle, $! is set as usual and 
